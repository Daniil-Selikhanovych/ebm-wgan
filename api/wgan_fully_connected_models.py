...
class Generator_fully_connected(nn.Module):
    def __init__(self, 
                 ...
                 non_linear = nn.ReLU()):
        super(Generator_fully_connected, self).__init__()
...
class Discriminator_fully_connected(nn.Module):
    def __init__(self, 
                 ...
                 device = device_default, 
                 non_linear = nn.ReLU()):
        super(Discriminator_fully_connected, self).__init__()
        self.non_linear = non_linear
        self.device = device
        self.n_hidden = n_hidden
        self.n_in = n_in
        self.layers = nn.ModuleList([nn.Linear(self.n_in, self.n_hidden),
                                     nn.Linear(self.n_hidden, self.n_hidden), 
                                     nn.Linear(self.n_hidden, self.n_hidden),
